...
{
    // Convert fluid:// to fluid:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("fluid://"))
    {
        uri.replace(0, 12, "fluid:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Fluid.lnk";
}
...
{
    return GetAutostartDir() / "fluid.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Fluid-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  fluid-qt [" + tr("command-line options") + "]                     " + "\n";
